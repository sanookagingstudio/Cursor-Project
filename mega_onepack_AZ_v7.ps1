...
$global:ONEPACK_LOG = Join-Path (Get-Location) "mega_onepack_AZ_v7.log"
...
$guard = Join-Path $root ".sas_guard_v7.json"

$fp = @{ root=$root; time=(Get-Date).ToString("o"); marker="SAS-FRONTEND-MIGRATION-v7" } | ConvertTo-Json
...
try{
    ...
    if(Test-Path ".git"){
        ...
        if($st){
            ...
            $msg="[ONEPACK-v7] Auto snapshot "+(Get-Date -f s)
...
"""API routers package (self-healing v7)."""
...
# PHASE F — FRONTEND AUTO-MIGRATION + DEV START + BACKEND DEV START
...
try{
    ...
    # 1) AUTO-MIGRATE FRONTEND ROOT → frontend/
    ...
    if(!(Test-Path $frontendNode) -and (Test-Path $frontendPkg)){
    ...
    # 3) START BACKEND DEV (uvicorn) — background

    LogFile $p "Starting backend dev server (uvicorn)..."

    Start-Process -WorkingDirectory $backend -FilePath "cmd.exe" -ArgumentList "/c python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload" -WindowStyle Minimized | Out-Null



    # 4) START FRONTEND DEV (npm run dev -- --port 3000) — background

    if(Test-Path $frontendPkg){

        LogFile $p "Starting frontend dev server (npm run dev -- --port 3000)..."

        Start-Process -WorkingDirectory $frontend -FilePath "cmd.exe" -ArgumentList "/c npm run dev -- --port 3000" -WindowStyle Minimized | Out-Null

    } else {

        LogFile $p "No frontend package.json found in frontend/, skip dev start"
    ...
    Summary $p "Backend & frontend dev started (v7)"

}catch{

    PhaseSet $p "PASS-WITH-ISSUES"

    Summary $p "Runtime start issue: $($_.Exception.Message)"
...
# PHASE G — HEALTH CHECK (BACKEND + FRONTEND, RETRIES)
...
$sum = Join-Path $root "mega_onepack_AZ_v7_summary.txt"



$txt=@()

$txt+="ONEPACK A→Z v7 Report"

$txt+="Time: "+(Get-Date -f s)

$txt+="Runtime: dev + frontend auto-migration + full auto fix"
