# ONE PACK MEGA PURE v6 – FINAL IMMORTAL MODE

# REST-only, no PAT, no pg-meta, idempotent and error-tolerant.



Write-Host "=== ONE PACK MEGA PURE v6 – FINAL IMMORTAL MODE START ===" -ForegroundColor Cyan
...
$ADMIN_EMAIL      = "admin@funaging.club"

$STAFF_EMAIL      = "staff@funaging.club"
...
function Get-AuthUserByEmail {
    ...
    $url = "$AUTH_URL/admin/users?email=$([uri]::EscapeDataString($Email))"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        $msg = "AUTO: OnePack Mega Pure v6 $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
