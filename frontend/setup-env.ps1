...
if (Test-Path $EnvFile) {
    Write-Host "📝 Updating existing .env.local..." -ForegroundColor Yellow
    ...
    if ($ExistingContent -match "NEXT_PUBLIC_API_URL=") {
        ...
        Write-Host "✅ Updated NEXT_PUBLIC_API_URL to $ApiUrl" -ForegroundColor Green
    } else {
        Add-Content -Path $EnvFile -Value "`nNEXT_PUBLIC_API_URL=$ApiUrl"
        Write-Host "✅ Added NEXT_PUBLIC_API_URL=$ApiUrl" -ForegroundColor Green
    }
} else {
    Write-Host "📝 Creating new .env.local..." -ForegroundColor Yellow
    Set-Content -Path $EnvFile -Value $EnvContent
    Write-Host "✅ Created .env.local with NEXT_PUBLIC_API_URL=$ApiUrl" -ForegroundColor Green
}

# Step 2: Fix multiple lockfiles warning
Write-Host "`n🔍 Checking for lockfile issues..." -ForegroundColor Cyan
...
if ($LockFilesFound.Count -gt 1) {
    Write-Host "⚠️  Multiple lockfiles detected: $($LockFilesFound -join ', ')" -ForegroundColor Yellow
    Write-Host "📦 Keeping package-lock.json (npm), removing others..." -ForegroundColor Yellow
    ...
    Write-Host "✅ Lockfile cleanup complete. Using npm (package-lock.json)" -ForegroundColor Green
} else {
    Write-Host "✅ No lockfile conflicts detected" -ForegroundColor Green
}
...
Write-Host "`n💡 To use a different API URL, run:" -ForegroundColor Cyan
