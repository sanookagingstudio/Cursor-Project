...
$global:ONEPACK_LOG = Join-Path (Get-Location) "mega_onepack_AZ_v2.log"
...
function GitSnap { param($p)
    ...
    if($s){
        ...
        $m = "[ONEPACK-AZ-v2] Auto snapshot "+(Get-Date -f s)

        git commit -m $m | Out-Null

        $t="onepack-az-v2-"+(Get-Date -f yyyyMMddHHmmss)
...
function UpDocker { param($f,$p)
    ...
    try {
        ...
        catch { LogFile $p "Docker up fail: "+$_.Exception.Message }
...
function Start-DockerDesktopIfPossible { param($p)
    ...
    for($i=1;$i -le 30;$i++){
        ...
        if(Test-DockerRunning){ LogFile $p "Docker engine is running."; return $true }
...
$guard=Join-Path $root ".sas_guard_v2.json"
...
try {
    ...
    $fp = @{ project_root=$root; time=(Get-Date).ToString("o"); marker="FunAging.club-SAS-v1"; version="ONEPACK-AZ-v2" } | ConvertTo-Json
...
try{
    ...
    if(Test-DockerRunning){
        ...
        LogFile $p "Docker engine is available. Using docker runtime."
        ...
        LogFile $p "Docker engine not running. Trying to start Docker Desktop..."
...
$sum=Join-Path $root "mega_onepack_AZ_v2_summary.txt"



$txt=@()

$txt+="ONEPACK A→Z v2 Report"
